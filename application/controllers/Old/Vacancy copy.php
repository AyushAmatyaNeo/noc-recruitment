    ...
    public function apply()
    {
        ...
        { 
            ...
            if($this->input->post('applySubmit'))
            {
                ...
                $applicantIdMax = $this->VacancyModel->getMaxIdapp();
                ...
                $RegData['applicant'] = array(
                    'APPLICATION_ID' => $applicantIdMax['MAXID']+1,
                    'AD_NO'  => strip_tags($this->input->post('AD_NO')),
                ...
                $RegData['Traning'] = [];
                    for($i=0; $i < count($data['TRAINING_NAME']); $i++)
                    {
                        ...
                        $RegData['Traning'][$i]['REG_ID'] = $RegId['MAXID']+1;
                ...
                { 
                    ...
                    if($this->fingerleftupload->do_upload('fingerright'))
                    {
                        ...
                        if($insert == true)
                        {
                            $data['details'] = $this->VacancyModel->details();
                            $data['user'] = $this->UserModel->getRows($con);
                            // echo '<pre>';print_r($data['user']); die;
                            $data['meta'] = array(
                            'title' => 'NOC | Vacancy Application Submitted',
                            'Description' => 'Vacancy Application'
                            );
